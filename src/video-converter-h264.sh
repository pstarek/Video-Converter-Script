...
ffmpeg -hide_banner -y -i "$filename" -aspect $aspect -target pal-dvd -vcodec copy -acodec copy 2>/dev/null - | ffmpeg -hide_banner -y -i - -vtag H264 -f avi -c:v libx264 -aspect $aspect -s $ssize -preset medium -crf 23 -profile:v main -level 3.1 -c:a libmp3lame -b:a 128000 -ar 44100 -ac 2 "$aviout"
